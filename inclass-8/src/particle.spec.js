...
describe('Particle Functionality', () => {
    ...
    it('particles should wrap around the world', () => {
        ...
        // canvas = { width, height }
		const pnorth = particle({ position: [width/2, -height/2] })
		const psouth = particle({ position: [width/2, 3*height/2] })
		const peast = particle({ position: [-width/2, height/2] })
		const pwest = particle({ position: [3*width/2, height/2] })
		const { positionn } = update(pnorth, 1.0)
		const { positions } = update(psouth, 1.0)
		const { positionw } = update(pwest, 1.0)
		const { positione } = update(peast, 1.0)

        expect(positionn).to.eql([width/2, height/2])
        expect(positions).to.eql([width/2, height/2])
        expect(positionw).to.eql([width/2, height/2])
        expect(positione).to.eql([width/2, height/2])
    })
