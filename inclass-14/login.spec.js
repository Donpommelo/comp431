...
describe('Test Dummy Server Example Page', () => {
    ...
    it("Update the headline and verify the change", (done) => {
        ...
        let oldHeadline = findId('headline').getText();

        sleep(500)
            .then(findId('headline').sendKeys("meep"))
            .then(findId('headline').getText()
                .then(text => {
                    expect(text.indexOf('meep')).to.equal(0)
                }))
            .then(findId('headline').sendKeys(oldHeadline))
            .then(findId('headline').getText()
                .then(text => {
                    expect(text.indexOf(oldHeadline)).to.equal(0)
                })
                .then(done))
    })
