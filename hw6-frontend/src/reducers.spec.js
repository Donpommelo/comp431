...
describe('Validate reducer', () =>  {
    ...
    it('should initialize state', (done) => {
        const startState = { "currentPage": "AUTH_PAGE", "account": {}, "message":'', "followers":[], "articles":[],
        "articlesCommenting":[], "articlesEditing":[], "commentsEditing":[], "keyword":''}
    ...
    it('should set the articles', (done) => {
        ...
        expect(reducers.articlesReducer([], {type:"NEW_ARTICLE", post: articles})).to.eql(articles)
        done()        
    })
    ...
    it('should filter displayed articles by the search keyword', (done) => {
        ...
        const keyword = 'test1'
        expect(reducers.articlesReducer(articles, {type:"FILTER", text: keyword})).to.eql(filteredarticles)
        done()    
    })
})
