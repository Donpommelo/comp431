...
export const linkAccount = (locusername, locpassword) => {
    return (dispatch) => {
        Actions.resource('PUT', 'link', { locusername , locpassword })
        .then(r => dispatch({ type: 'SUCCESS', message: 'Accounts Linked!'}))
        .catch(r => dispatch({ type: 'ERROR', message: 'Accounts not Linked!'}))
    }
}

export const unlinkAccount = () => {
    return (dispatch) => {
        Actions.resource('GET', 'unlink')
        .then(r => dispatch({ type: 'SUCCESS', message: 'Accounts Unlinked!'}))
        .catch(r => dispatch({ type: 'ERROR', message: 'Accounts not Unlinked!'}))
    }
}